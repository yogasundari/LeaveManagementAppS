...
@Service
public class LateLeaveScheduler {
    ...
    public LateLeaveScheduler(EmployeeRepository employeeRepo,
    ...
    public void adjustMonthlyPermissionBalances() {
        ...
        LeaveType lateType = leaveTypeRepo.findByTypeNameIgnoreCase("late")
                .orElseThrow(() -> new IllegalStateException("Late LeaveType not found"));

        // 5) For each employee, count used and deduct unused
        for (Employee emp : employeeRepo.findAll()) {
            int used = lrRepo.countlateInRange(
                    emp.getEmpId(),
            ...
            EmployeeLeaveBalance bal = balanceRepo
                    .findByEmployeeAndLeaveType(emp, lateType)
