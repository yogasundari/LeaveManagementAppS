...
import com.saveetha.LeaveManagement.exception.LeaveValidationException;
...
public class LeaveValidationService {
    ...
    public void validateMedicalLeave(LeaveRequestDTO dto) {
        ...
        if (requestedDays < 3) {
            throw new LeaveValidationException("Medical leave must be taken for 3 or more consecutive days.");
        }

        // Step 2: Ensure a medical certificate is uploaded
        if (dto.getFileUpload() == null || dto.getFileUpload().isEmpty()) {
            throw new LeaveValidationException("Medical leave requires a medical certificate to be uploaded.");
        }
        ...
        LeaveType mlLeaveType = leaveTypeRepository.findByTypeNameIgnoreCase("ml")
                .orElseThrow(() -> new LeaveValidationException("Medical Leave type not found"));
        ...
        EmployeeLeaveBalance balance = employeeLeaveBalanceRepository
                ...
                .orElseThrow(() -> new LeaveValidationException("Medical Leave balance not found"));

        // Step 5: Prevent drafting if balance is zero
        if (balance.getBalanceLeave() <= 0) {
            throw new LeaveValidationException("You have no remaining Medical Leave balance.");
        }
        ...
        if (!overlappingRequests.isEmpty()) {
            throw new LeaveValidationException("Overlapping Medical Leave request exists for the selected dates.");
        }
        ...
        if (futureTotal > 6) {
            throw new LeaveValidationException("This request exceeds the allowed Medical Leave limit (6 days). " +
    ...
    public void validateEarnedLeave(LeaveRequestDTO dto) {
        ...
        long requestedDays = ChronoUnit.DAYS.between(dto.getStartDate(), dto.getEndDate()) + 1;
        if (requestedDays < 3) {
            throw new LeaveValidationException("Earned Leave must be taken for 3 or more consecutive days.");
        }
        ...
        EmployeeLeaveBalance balance = employeeLeaveBalanceRepository
                ...
                .orElseThrow(() -> new LeaveValidationException("Earned Leave balance not found"));

        if (balance.getBalanceLeave() <= 0) {
            throw new LeaveValidationException("You have no remaining Earned Leave balance.");
        }
        ...
        if (!overlappingRequests.isEmpty()) {
            throw new LeaveValidationException("Overlapping Earned Leave request exists for the selected dates.");
        }
        ...
        if (futureTotal > 12) {
            throw new LeaveValidationException("This request exceeds the allowed Earned Leave limit (12 days). " +
    ...
    public void validateCompOff(LeaveRequestDTO dto) {
        ...
        if (dto.getEarnedDate() == null) {
            throw new LeaveValidationException("Comp Off leave requires an earned date.");
        }

        // Step 2: Earned date must be before the leave start date
        if (!dto.getEarnedDate().isBefore(dto.getStartDate())) {
            throw new LeaveValidationException("Earned Date must be before the leave Start Date.");
        }
        ...
        if (!existingEarnedDateRequests.isEmpty()) {
            throw new LeaveValidationException("The earned date you have already used");
        }
        ...
        if (!overlappingRequests.isEmpty()) {
            throw new LeaveValidationException("Overlapping Comp Off leave request exists for the selected dates.");
        }
    }
    public void validateCasualLeave(LeaveRequestDTO dto) {
        ...
        if (clUsed + requestedCLDays > completedMonths) {
            throw new LeaveValidationException("CL limit exceeded. You have " + completedMonths + " CL(s) available, used " + clUsed + ", trying to use " + requestedCLDays + ".");
        }
        ...
        if (!overlappingRequests.isEmpty()) {
            throw new LeaveValidationException("A CL request already exists for the selected date range.");
        }
    }
...
// Step 1: Retrieve Vacation leave type
    ...
    public void validatelate(LeaveRequestDTO leaveRequestDTO){
        ...
        if (bal.getBalanceLeave() <= 0) {
            throw new LeaveValidationException("You have no remaining Late Leave balance.");
        }

        // — Step 5: Check overlapping leaves
        boolean hasOverlap = leaveRequestRepository.existsByEmployeeAndDate(emp.getEmpId(), day);
        if (hasOverlap) {
            throw new LeaveValidationException("You already have a leave on " + day);
        }
        ...
        MonthRange currentMonth = ranges.stream()
                .filter(r -> !day.isBefore(r.getStart()) && !day.isAfter(r.getEnd()))
                .findFirst()
                .orElseThrow(() -> new LeaveValidationException("Date not in any academic month"));
